...
def get_pharma_class(rxcui, d):
	...
	if 'rxclassDrugInfoList' not in r:
		return None
	d['Pharm_Class_1'] = r["rxclassDrugInfoList"]["rxclassDrugInfo"][0]["rxclassMinConceptItem"]["className"]
	class_id = r["rxclassDrugInfoList"]["rxclassDrugInfo"][0]["rxclassMinConceptItem"]["classId"]
	return class_id
...
def calculate_MME(rxcui, d):
	...
	r = response.json()
	mme = 0
	for ingredient in r['rxcuiStatusHistory']['definitionalFeatures']['ingredientAndStrength']:
		...
		if baseName.lower() == 'buprenorphine':
			...
			doseForms = []
			try:
				doseForms = [x['doseFormGroupName'] for x in r['rxcuiStatusHistory']['definitionalFeatures']['doseFormGroupConcept']]
			except:
				pass
			formConcepts = []
			try:
				formConcepts = [x['doseFormName'] for x in r['rxcuiStatusHistory']['definitionalFeatures']['doseFormConcept']]
			except:
				pass
			if(ingredient['denominatorUnit'] == 'HR'):
